  ...
  async fetch(request, env) {
    ...
    try {
      if (url.pathname === '/' || url.pathname === '/health') {
        return json({ok:true,service:'Genesis 3D Model Bridge',version:4,capabilities:{makerworld:true,thingiverse:!!String(env.THINGIVERSE_ACCESS_TOKEN||'').trim()},time:new Date().toISOString()},200,origin);
      }
